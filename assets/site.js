...
function removeRetiredDemoArticles(){
  const retiredSlugs = ['visual-elements-feature-showcase'];
  try {
    const dynamics = JSON.parse(localStorage.getItem('ashishJournal:dynamicArticles') || '[]');
    const filteredDynamics = dynamics.filter(article => !retiredSlugs.includes(article.slug));
    if(filteredDynamics.length !== dynamics.length){
      localStorage.setItem('ashishJournal:dynamicArticles', JSON.stringify(filteredDynamics));
    }

    const deleted = JSON.parse(localStorage.getItem('ashishJournal:deletedArticles') || '[]');
    const mergedDeleted = [...new Set([...deleted, ...retiredSlugs])];
    if(mergedDeleted.length !== deleted.length){
      localStorage.setItem('ashishJournal:deletedArticles', JSON.stringify(mergedDeleted));
    }

    localStorage.setItem('ashishJournal:showcaseSeeded', 'yes');
  } catch {
    localStorage.setItem('ashishJournal:deletedArticles', JSON.stringify(retiredSlugs));
    localStorage.setItem('ashishJournal:showcaseSeeded', 'yes');
  }
}
